...
class DataBowl3Detector(Dataset):
    def __init__(self, filename, split_path, config, phase = 'train',split_comber=None):
	...
	#idcs = ['0015ceb851d7251b8f399e39779d1e7d']
        if phase!='test':
            idcs = [f for f in idcs if (f not in self.blacklist)]

        #self.filenames = [os.path.join(data_dir, '%s_clean.nii' % idx) for idx in idcs]
    ...
    def __getitem__(self, idx,split=None):
	    ...
	    imgs = nib.load(self.filenames[idx]).get_data()
